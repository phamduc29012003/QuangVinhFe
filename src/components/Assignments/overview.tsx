import { STATUS_ICON, STATUS_LABEL, type TaskRow } from './ProjectDetailTable/columns'

export const Overview = (props: { tasks: TaskRow[] }) => {
  const { tasks } = props
  return (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="flex items-center justify-between rounded-md border border-gray-100 bg-gray-50 p-3">
            ...
            <div className="text-lg font-semibold">
              {tasks.reduce((sum, t) => sum + (t.estimateHours || 0), 0)} h
            </div>
          </div>
        </div>

        <div className="mt-3 flex flex-wrap gap-2">
          {(['todo', 'in_progress', 'pending', 'done', 'cancel'] as TaskRow['status'][]).map(
            (st) => {
